...
import 'package:http/http.dart' as http; // IMPORTANTE: agregar 'as http'
...
class ArticleModel {
  Future<Summary> getRandomArticleSummary() async {
    ...
    final response = await http.get(uri); // Corregido: http.get

    if (response.statusCode != 200) {
      throw HttpException('Failed to update resource: ${response.statusCode}');
    }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        spacing: 10.0,
        children: [
          // Versión simplificada que también funciona
